...
public class AnbarData {
    ...
    private PreparedStatement getQebulcuName;

    private PreparedStatement updateXammalAnbarItem;
    private PreparedStatement deleteXammalAnbarITem;
    private PreparedStatement updateXammalSiyahi;
    ...
    private static final String GET_QEBULCU_NAME="SELECT " + COLUMN_MEXARIC_TEHVILALAN + " FROM " +
            TABLE_MEXARIC + " WHERE " + COLUMN_MEXARIC_NR+ " = ?";
    ...
    private static final String UPDATE_XAMMAL_ITEM="UPDATE " + TABLE_ANBAR  + " SET " + COLUMN_ANBAR_CEKI + " = ? "  +  ", " + COLUMN_ANBAR_mebleg + " = ?"  +
            " WHERE "  + COLUMN_ANBAR_NR + " = ?";

    private static final String UPDATE_XAMMAL_SIYAHI= "UPDATE " + TABLE_MAL + " SET " + COLUMN_MAL_ORTAQIYMET + " = ?" + " WHERE " +  COLUMN_MAL_NR + " = ?";
    ...
    public void open(){
        try {
            ...
            getQebulcuName=connection.prepareStatement(GET_QEBULCU_NAME);
            updateXammalAnbarItem = connection.prepareStatement(UPDATE_XAMMAL_ITEM);
            updateXammalSiyahi=connection.prepareStatement(UPDATE_XAMMAL_SIYAHI);
    ...
    public void close(){
        try {
            ...
            }if (getQebulcuName!=null){
                getQebulcuName.close();
            }if (updateXammalSiyahi!=null){
                updateXammalSiyahi.close();
            }
        }catch (SQLException e){
    ...
    public XammalMal getMal(String malAdi){
        try {
            queryMal.setString(1,malAdi);
            ResultSet resultSet= queryMal.executeQuery();
            if (resultSet.next()){
                XammalMal xammalMal = new XammalMal();
                xammalMal.setNr(resultSet.getInt(1));
                xammalMal.setAd(resultSet.getString(2));
                xammalMal.setVahid(resultSet.getString(3));
                xammalMal.setOrtaGiymet(resultSet.getDouble(4));
                return xammalMal;
            }
            return null;
        }catch (SQLException e){
    ...
    public boolean insertAnbar(String malAdi, double ceki, double mebleg){
        ...
        try {
            ...
            if (doesExistAnbar(malAdi)){
                ...
                if (nrAffectedRows==1){
                    connection.commit();
                    XammalMal xammalMal = getMal(malAdi);
                    double median= (xammalMal.getOrtaGiymet() + (mebleg/ceki)) /2;
                    updateMalValue.setDouble(1,median);
                    updateMalValue.setInt(2, xammalMal.getNr());
                    updateMalValue.executeUpdate();
                    done=true;
                }
            }else{
                XammalMal xammalMal = getMal(malAdi);
                insertAnbar.setString(1, xammalMal.getAd());
                insertAnbar.setString(2, xammalMal.getVahid());
    ...
    public void insertMedaxilFaktura(String malAdi,double ceki,double mebleg,int medaxilNr){
        ...
        try {
            connection.setAutoCommit(false);
            XammalMal xammalMal =getMal(malAdi);
                insertMedaxilFaktura.setInt(1, xammalMal.getNr());
                insertMedaxilFaktura.setString(2, xammalMal.getAd());
                insertMedaxilFaktura.setString(3, xammalMal.getVahid());
    ...
    public boolean insertMexaricFaktura(String malAdi,double ceki,int medaxilNr){
        ...
        try {
            connection.setAutoCommit(false);
            XammalMal xammalMal =getMal(malAdi);
            insertMexaricFaktura.setInt(1, xammalMal.getNr());
            insertMexaricFaktura.setString(2, xammalMal.getAd());
            insertMexaricFaktura.setString(3, xammalMal.getVahid());
    ...
    private boolean decreaseAnbar(String item,double ceki){
        ...
        try {
            connection.setAutoCommit(false);
            XammalMal xammalMal = getMal(item);
            decreaseAnbar.setDouble(1,ceki);
            AnbarItem anbarItem = getItem(item);
            double yekunMebleg= anbarItem.getMebleg() - (ceki* xammalMal.getOrtaGiymet());
    ...
    public List<XammalMal> queryAllMal(){
        try (Statement statement=connection.createStatement();
        ResultSet resultSet=statement.executeQuery(QUERY_ALL_MAL)){
            List<XammalMal> xammalMalList = new ArrayList<>();
            while (resultSet.next()){
                XammalMal xammalMal = new XammalMal();
                xammalMal.setNr(resultSet.getInt(1));
                xammalMal.setAd(resultSet.getString(2));
    ...
    public String getGebulcuName(int nr){
        try {
            getQebulcuName.setInt(1,nr);
            ResultSet resultSet = getQebulcuName.executeQuery();
            return resultSet.getString(1);
        }catch (SQLException e){
            System.out.println(e.getMessage());
            return null;
    ...
    public void updateXammalAnbarItem(int id,double ceki,double amount){
        try {
            connection.setAutoCommit(false);
            updateXammalAnbarItem.setDouble(1,ceki);
            updateXammalAnbarItem.setDouble(2,amount);
            updateXammalAnbarItem.setInt(3,id);
            int nrAffectedRows = updateXammalAnbarItem.executeUpdate();
            if (nrAffectedRows == 1) {
                connection.commit();
    ...
    public void updateXammalSiyahi(int malNr,double value){
        try {
            updateXammalSiyahi.setDouble(1,value);
            updateXammalSiyahi.setInt(2,malNr);
            updateXammalSiyahi.executeUpdate();
        }catch (Exception e){
            e.printStackTrace();
        }
    }
